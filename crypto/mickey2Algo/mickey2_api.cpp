...
int encrypt_mickey2(const char* input, const char* key, char* output, int bufferSize) {
...
int decrypt_mickey2(const char* input, const char* key, char* output, int bufferSize) {
    if (!input || !key || !output || bufferSize <= 1) {
        return 0;
    }
